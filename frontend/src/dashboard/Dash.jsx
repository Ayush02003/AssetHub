import { useState } from "react";
import "../css/dash.css";
import { NavLink, useLocation } from "react-router-dom";
import {
  // BrowserRouter as Router,
  Routes,
  Route,
  Navigate,
} from "react-router-dom";

import Home from "./dash_pages/Home.jsx";
import Users from "./dash_pages/Users.jsx";
import Total_asset from "./dash_pages/Total_asset.jsx";
import Profile from "./dash_pages/Profile.jsx";
import Request_asset from "./dash_pages/Request_asset.jsx";
import Request_maintenance from "./dash_pages/Request_maintenance.jsx";
import Asset_return from "./dash_pages/Asset_return.jsx";
import Asset_allocation from "./dash_pages/Asset_allocation.jsx";
import Add_asset from "./dash_pages/Add_asset.jsx";
import Add_software from "./dash_pages/Add_software.jsx";
import Add_dept from "./dash_pages/Add_dept.jsx";
import Add_employee from "./dash_pages/Add_employee.jsx";
import Add_asset_type from "./dash_pages/Add_asset_type.jsx";
import Temp_allocation from "./dash_pages/Temp_allocation.jsx";
import Feedback from "./dash_pages/Feedback.jsx";
import Notification from "./dash_pages/Notification.jsx";
import Setting from "./dash_pages/Setting.jsx";
import HR_Notification_detail from "./dash_pages/inner_pages/hr_notification_detail.jsx";
import IT_Notification_detail from "./dash_pages/inner_pages/it_notification_detail.jsx";
import IT_return_detail from "./dash_pages/inner_pages/it_return_detail.jsx";
import IT_issue_detail from "./dash_pages/inner_pages/it_issue_detail.jsx";
import Emp_Notification_detail from "./dash_pages/inner_pages/emp_notification_detail.jsx";
import Emp_issue_detail from "./dash_pages/inner_pages/emp_issue_detail.jsx";
import Emp_return_detail from "./dash_pages/inner_pages/emp_return_detail.jsx";

import User_detail from "./dash_pages/inner_pages/user_detail.jsx";
import Asset_detail from "./dash_pages/inner_pages/asset_detail.jsx";
import { useAuthContext } from "../context/AuthContext.jsx";
import useLogout from "../hooks/UseLogout.js";
import ProtectedRoute from "./protected_routes/Protected_Routes.jsx";
import Departments from "./dash_pages/Departments.jsx";
import Asset_type from "./dash_pages/Asset_type.jsx";
import Request_software from "./dash_pages/Request_software.jsx";
const Dash = () => {
  const location = useLocation();
  const { loading, logout } = useLogout();
  const { pathname } = location;
  const { authUser } = useAuthContext();
  const [openDash, setopenDash] = useState(false);
  const [openForms, setopenForms] = useState(false);
  return (
    <div className="dashboard">
      <div className="sidebar">
        <div className="upper-bar">
          <h1>AssetHub</h1>
        </div>
        <div className="lower-bar">
          <div className="options">
            <h3 className="mb-4 ml-4 text-sm font-semibold text-bodydark2">
              MENU
            </h3>
            <ul>
              <NavLink to="/dashboard/home">
                <li
                  onClick={() => setopenDash(!openDash)}
                  className={`${
                    (pathname === "/home" || pathname.includes("home")) &&
                    "selected"
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="18"
                    height="18"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M6.10322 0.956299H2.53135C1.5751 0.956299 0.787598 1.7438 0.787598 2.70005V6.27192C0.787598 7.22817 1.5751 8.01567 2.53135 8.01567H6.10322C7.05947 8.01567 7.84697 7.22817 7.84697 6.27192V2.72817C7.8751 1.7438 7.0876 0.956299 6.10322 0.956299ZM6.60947 6.30005C6.60947 6.5813 6.38447 6.8063 6.10322 6.8063H2.53135C2.2501 6.8063 2.0251 6.5813 2.0251 6.30005V2.72817C2.0251 2.44692 2.2501 2.22192 2.53135 2.22192H6.10322C6.38447 2.22192 6.60947 2.44692 6.60947 2.72817V6.30005Z"
                      fill=""
                    />
                    <path
                      d="M15.4689 0.956299H11.8971C10.9408 0.956299 10.1533 1.7438 10.1533 2.70005V6.27192C10.1533 7.22817 10.9408 8.01567 11.8971 8.01567H15.4689C16.4252 8.01567 17.2127 7.22817 17.2127 6.27192V2.72817C17.2127 1.7438 16.4252 0.956299 15.4689 0.956299ZM15.9752 6.30005C15.9752 6.5813 15.7502 6.8063 15.4689 6.8063H11.8971C11.6158 6.8063 11.3908 6.5813 11.3908 6.30005V2.72817C11.3908 2.44692 11.6158 2.22192 11.8971 2.22192H15.4689C15.7502 2.22192 15.9752 2.44692 15.9752 2.72817V6.30005Z"
                      fill=""
                    />
                    <path
                      d="M6.10322 9.92822H2.53135C1.5751 9.92822 0.787598 10.7157 0.787598 11.672V15.2438C0.787598 16.2001 1.5751 16.9876 2.53135 16.9876H6.10322C7.05947 16.9876 7.84697 16.2001 7.84697 15.2438V11.7001C7.8751 10.7157 7.0876 9.92822 6.10322 9.92822ZM6.60947 15.272C6.60947 15.5532 6.38447 15.7782 6.10322 15.7782H2.53135C2.2501 15.7782 2.0251 15.5532 2.0251 15.272V11.7001C2.0251 11.4188 2.2501 11.1938 2.53135 11.1938H6.10322C6.38447 11.1938 6.60947 11.4188 6.60947 11.7001V15.272Z"
                      fill=""
                    />
                    <path
                      d="M15.4689 9.92822H11.8971C10.9408 9.92822 10.1533 10.7157 10.1533 11.672V15.2438C10.1533 16.2001 10.9408 16.9876 11.8971 16.9876H15.4689C16.4252 16.9876 17.2127 16.2001 17.2127 15.2438V11.7001C17.2127 10.7157 16.4252 9.92822 15.4689 9.92822ZM15.9752 15.272C15.9752 15.5532 15.7502 15.7782 15.4689 15.7782H11.8971C11.6158 15.7782 11.3908 15.5532 11.3908 15.272V11.7001C11.3908 11.4188 11.6158 11.1938 11.8971 11.1938H15.4689C15.7502 11.1938 15.9752 11.4188 15.9752 11.7001V15.272Z"
                      fill=""
                    />
                  </svg>
                  <span>Dashboard</span>
                  {/* <i
                  className={`fas fa-angle-${openDash ? "down" : "up"}`}
                  aria-hidden="true"
                ></i> */}
                </li>
              </NavLink>
              {authUser.role != "Employee" ? (
                <>
                  <NavLink to="/dashboard/users">
                    <li
                      className={`${
                        (pathname === "/users" || pathname.includes("users")) &&
                        "selected"
                      }`}
                    >
                      <i className="fa fa-users" aria-hidden="true"></i>
                      Users
                    </li>
                  </NavLink>
                  <NavLink to="/dashboard/total_asset">
                    <li
                      className={`${
                        (pathname === "/total_asset" ||
                          pathname.includes("total_asset")) &&
                        "selected"
                      }`}
                    >
                      <svg
                        className="fill-current"
                        width="18"
                        height="19"
                        viewBox="0 0 18 19"
                        fill="none"
                        xmlns="http://www.w3.org/2000/svg"
                      >
                        <g clipPath="url(#clip0_130_9756)">
                          <path
                            d="M15.7501 0.55835H2.2501C1.29385 0.55835 0.506348 1.34585 0.506348 2.3021V15.8021C0.506348 16.7584 1.29385 17.574 2.27822 17.574H15.7782C16.7345 17.574 17.5501 16.7865 17.5501 15.8021V2.3021C17.522 1.34585 16.7063 0.55835 15.7501 0.55835ZM6.69385 10.599V6.4646H11.3063V10.5709H6.69385V10.599ZM11.3063 11.8646V16.3083H6.69385V11.8646H11.3063ZM1.77197 6.4646H5.45635V10.5709H1.77197V6.4646ZM12.572 6.4646H16.2563V10.5709H12.572V6.4646ZM2.2501 1.82397H15.7501C16.0313 1.82397 16.2563 2.04897 16.2563 2.33022V5.2271H1.77197V2.3021C1.77197 2.02085 1.96885 1.82397 2.2501 1.82397ZM1.77197 15.8021V11.8646H5.45635V16.3083H2.2501C1.96885 16.3083 1.77197 16.0834 1.77197 15.8021ZM15.7501 16.3083H12.572V11.8646H16.2563V15.8021C16.2563 16.0834 16.0313 16.3083 15.7501 16.3083Z"
                            fill=""
                          />
                        </g>
                        <defs>
                          <clipPath id="clip0_130_9756">
                            <rect
                              width="18"
                              height="18"
                              fill="white"
                              transform="translate(0 0.052124)"
                            />
                          </clipPath>
                        </defs>
                      </svg>
                      <span>Asset</span>
                    </li>
                  </NavLink>
                </>
              ) : (
                ""
              )}
              {authUser.role === "Admin" && (
                <>
                 <NavLink to="/dashboard/asset_type">
                 <li
                   className={`${
                     (pathname === "/asset_type" || pathname.includes("asset_type")) &&
                     "selected"
                   }`}
                 >
                   <i className="fa fa-cubes"></i>
                   <span>Asset Types</span>
                   {/* <i
                   className={`fas fa-angle-${openDash ? "down" : "up"}`}
                   aria-hidden="true"
                 ></i> */}
                 </li>
               </NavLink>
               <NavLink to="/dashboard/departments">
                <li
                  // onClick={() => setopenDash(!openDash)}
                  className={`${
                    (pathname === "/departments" || pathname.includes("departments")) &&
                    "selected"
                  }`}
                >
                  <i className="fa fa-building"></i>
                  <span>Departments</span>
                  {/* <i
                  className={`fas fa-angle-${openDash ? "down" : "up"}`}
                  aria-hidden="true"
                ></i> */}
                </li>
              </NavLink>
              </>
              )}
              <li onClick={() => setopenForms(!openForms)}>
                <svg
                  className="fill-current"
                  width="18"
                  height="18"
                  viewBox="0 0 18 18"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M1.43425 7.5093H2.278C2.44675 7.5093 2.55925 7.3968 2.58737 7.31243L2.98112 6.32805H5.90612L6.27175 7.31243C6.328 7.48118 6.46862 7.5093 6.58112 7.5093H7.453C7.76237 7.48118 7.87487 7.25618 7.76237 7.03118L5.428 1.4343C5.37175 1.26555 5.3155 1.23743 5.14675 1.23743H3.88112C3.76862 1.23743 3.59987 1.29368 3.57175 1.4343L1.153 7.08743C1.0405 7.2843 1.20925 7.5093 1.43425 7.5093ZM4.47175 2.98118L5.3155 5.17493H3.59987L4.47175 2.98118Z"
                    fill=""
                  />
                  <path
                    d="M10.1249 2.5031H16.8749C17.2124 2.5031 17.5218 2.22185 17.5218 1.85623C17.5218 1.4906 17.2405 1.20935 16.8749 1.20935H10.1249C9.7874 1.20935 9.47803 1.4906 9.47803 1.85623C9.47803 2.22185 9.75928 2.5031 10.1249 2.5031Z"
                    fill=""
                  />
                  <path
                    d="M16.8749 6.21558H10.1249C9.7874 6.21558 9.47803 6.49683 9.47803 6.86245C9.47803 7.22808 9.75928 7.50933 10.1249 7.50933H16.8749C17.2124 7.50933 17.5218 7.22808 17.5218 6.86245C17.5218 6.49683 17.2124 6.21558 16.8749 6.21558Z"
                    fill=""
                  />
                  <path
                    d="M16.875 11.1656H1.77187C1.43438 11.1656 1.125 11.4469 1.125 11.8125C1.125 12.1781 1.40625 12.4594 1.77187 12.4594H16.875C17.2125 12.4594 17.5219 12.1781 17.5219 11.8125C17.5219 11.4469 17.2125 11.1656 16.875 11.1656Z"
                    fill=""
                  />
                  <path
                    d="M16.875 16.1156H1.77187C1.43438 16.1156 1.125 16.3969 1.125 16.7625C1.125 17.1281 1.40625 17.4094 1.77187 17.4094H16.875C17.2125 17.4094 17.5219 17.1281 17.5219 16.7625C17.5219 16.3969 17.2125 16.1156 16.875 16.1156Z"
                    fill="white"
                  />
                </svg>
                <span>Forms</span>
                <i
                  className={`fas fa-angle-${openForms ? "down" : "up"}`}
                  aria-hidden="true"
                ></i>
              </li>
              {openForms && (
                <>
                  {authUser.role == "Employee" ? (
                    <NavLink to="/dashboard/request_asset">
                      <li
                        className={`innerComp  ${
                          (pathname === "/request_asset" ||
                            pathname.includes("request_asset")) &&
                          "inner-selected"
                        }`}
                      >
                        Request Asset
                      </li>
                    </NavLink>
                  ) : (
                    ""
                  )}
                  {authUser.role == "Employee" ? (
                    <NavLink to="/dashboard/request_maintenance">
                      <li
                        className={`innerComp  ${
                          (pathname === "/request_maintenance" ||
                            pathname.includes("request_maintenance")) &&
                          "inner-selected"
                        }`}
                      >
                       Asset Help
                      </li>
                    </NavLink>
                  ) : (
                    ""
                  )}
                     {authUser.role == "Employee" ? (
                    <NavLink to="/dashboard/request_software">
                      <li
                        className={`innerComp  ${
                          (pathname === "/request_software" ||
                            pathname.includes("request_software")) &&
                          "inner-selected"
                        }`}
                      >
                        Request Software
                      </li>
                    </NavLink>
                  ) : (
                    ""
                  )}
                   {authUser.role == "Employee" ? (
                    <NavLink to="/dashboard/asset_return">
                      <li
                        className={`innerComp  ${
                          (pathname === "/asset_return" ||
                            pathname.includes("asset_return")) &&
                          "inner-selected"
                        }`}
                      >
                       Return Asset
                      </li>
                    </NavLink>
                  ) : (
                    ""
                  )}
                  {authUser.role == "HR" || authUser.role == "Admin" ? (
                    <NavLink to="/dashboard/add_employee">
                      <li
                        className={`innerComp  ${
                          (pathname === "/add_employee" ||
                            pathname.includes("add_employee")) &&
                          "inner-selected"
                        }`}
                      >
                        Add Employee
                      </li>
                    </NavLink>
                  ) : (
                    " "
                  )}
                  {authUser.role == "HR" || authUser.role == "Admin" ? (
                    <NavLink to="/dashboard/add_asset">
                      <li
                        className={`innerComp  ${
                          (pathname === "/add_asset" ||
                            pathname.includes("add_asset")) &&
                          "inner-selected"
                        }`}
                      >
                        Add Asset
                      </li>
                    </NavLink>
                  ) : (
                    " "
                  )}

                  {authUser.role == "IT-Person" ? (
                    <NavLink to="/dashboard/add_software">
                      <li
                        className={`innerComp  ${
                          (pathname === "/add_software" ||
                            pathname.includes("add_software")) &&
                          "inner-selected"
                        }`}
                      >
                        Add Software
                      </li>
                    </NavLink>
                  ) : (
                    " "
                  )}

                  {/* {authUser.role == "HR" || authUser.role == "Admin" ? (
                    <NavLink to="/dashboard/asset_allocation">
                      <li
                        className={`innerComp  ${
                          (pathname === "/asset_allocation" ||
                            pathname.includes("asset_allocation")) &&
                          "inner-selected"
                        }`}
                      >
                        Asset Allocation
                      </li>
                    </NavLink>
                  ) : (
                    " "
                  )} */}
                  {authUser.role == "Admin" ? (
                    <>
                    <NavLink to="/dashboard/add_AssetType">
                      <li
                        className={`innerComp  ${
                          (pathname === "/add_AssetType" ||
                            pathname.includes("add_AssetType")) &&
                          "inner-selected"
                        }`}
                      >
                        Add Asset Type
                      </li>
                    </NavLink>
                    <NavLink to="/dashboard/add_dept">
                    <li
                      className={`innerComp  ${
                        (pathname === "/add_dept" ||
                          pathname.includes("add_dept")) &&
                        "inner-selected"
                      }`}
                    >
                      Add Department
                    </li>
                  </NavLink>
                  </>
                  ) : (
                    ""
                  )}
                  {authUser.role == "IT-Person" ? (
                    <NavLink to="/dashboard/temp_allocation">
                      <li
                        className={`innerComp  ${
                          (pathname === "/temp_allocation" ||
                            pathname.includes("temp_allocation")) &&
                          "inner-selected"
                        }`}
                      >
                        Temporary Allocation
                      </li>
                    </NavLink>
                  ) : (
                    ""
                  )}
                  {authUser.role != "Admin" ? (
                    <NavLink to="/dashboard/feedback">
                      <li
                        className={`innerComp  ${
                          (pathname === "/feedback" ||
                            pathname.includes("feedback")) &&
                          "inner-selected"
                        }`}
                      >
                        Feedback
                      </li>
                    </NavLink>
                  ) : (
                    ""
                  )}
                </>
              )}

              <NavLink to="/dashboard/profile">
                <li
                  className={`${
                    (pathname === "/profile" || pathname.includes("profile")) &&
                    "selected"
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="18"
                    height="18"
                    viewBox="0 0 18 18"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <path
                      d="M9.0002 7.79065C11.0814 7.79065 12.7689 6.1594 12.7689 4.1344C12.7689 2.1094 11.0814 0.478149 9.0002 0.478149C6.91895 0.478149 5.23145 2.1094 5.23145 4.1344C5.23145 6.1594 6.91895 7.79065 9.0002 7.79065ZM9.0002 1.7719C10.3783 1.7719 11.5033 2.84065 11.5033 4.16252C11.5033 5.4844 10.3783 6.55315 9.0002 6.55315C7.62207 6.55315 6.49707 5.4844 6.49707 4.16252C6.49707 2.84065 7.62207 1.7719 9.0002 1.7719Z"
                      fill=""
                    />
                    <path
                      d="M10.8283 9.05627H7.17207C4.16269 9.05627 1.71582 11.5313 1.71582 14.5406V16.875C1.71582 17.2125 1.99707 17.5219 2.3627 17.5219C2.72832 17.5219 3.00957 17.2407 3.00957 16.875V14.5406C3.00957 12.2344 4.89394 10.3219 7.22832 10.3219H10.8564C13.1627 10.3219 15.0752 12.2063 15.0752 14.5406V16.875C15.0752 17.2125 15.3564 17.5219 15.7221 17.5219C16.0877 17.5219 16.3689 17.2407 16.3689 16.875V14.5406C16.2846 11.5313 13.8377 9.05627 10.8283 9.05627Z"
                      fill=""
                    />
                  </svg>
                  <span>Profile</span>
                </li>
              </NavLink>
              <NavLink to="/dashboard/notification">
                <li
                  className={`${
                    (pathname === "/notification" ||
                      pathname.includes("notification")) &&
                    "selected"
                  }`}
                >
                  <i className="fas fa-commenting" aria-hidden="true"></i>
                  Notification
                </li>
              </NavLink>

              <NavLink to="/dashboard/setting">
                <li
                  className={`${
                    (pathname === "/setting" || pathname.includes("setting")) &&
                    "selected"
                  }`}
                >
                  <svg
                    className="fill-current"
                    width="18"
                    height="19"
                    viewBox="0 0 18 19"
                    fill="none"
                    xmlns="http://www.w3.org/2000/svg"
                  >
                    <g clipPath="url(#clip0_130_9763)">
                      <path
                        d="M17.0721 7.30835C16.7909 6.99897 16.3971 6.83022 15.9752 6.83022H15.8909C15.7502 6.83022 15.6377 6.74585 15.6096 6.63335C15.5815 6.52085 15.5252 6.43647 15.4971 6.32397C15.4409 6.21147 15.4971 6.09897 15.5815 6.0146L15.6377 5.95835C15.9471 5.6771 16.1159 5.28335 16.1159 4.86147C16.1159 4.4396 15.9752 4.04585 15.6659 3.73647L14.569 2.61147C13.9784 1.99272 12.9659 1.9646 12.3471 2.58335L12.2627 2.6396C12.1784 2.72397 12.0377 2.7521 11.8971 2.69585C11.7846 2.6396 11.6721 2.58335 11.5315 2.55522C11.3909 2.49897 11.3065 2.38647 11.3065 2.27397V2.13335C11.3065 1.26147 10.6034 0.55835 9.73148 0.55835H8.15648C7.7346 0.55835 7.34085 0.7271 7.0596 1.00835C6.75023 1.31772 6.6096 1.71147 6.6096 2.10522V2.21772C6.6096 2.33022 6.52523 2.44272 6.41273 2.49897C6.35648 2.5271 6.32835 2.5271 6.2721 2.55522C6.1596 2.61147 6.01898 2.58335 5.9346 2.49897L5.87835 2.4146C5.5971 2.10522 5.20335 1.93647 4.78148 1.93647C4.3596 1.93647 3.96585 2.0771 3.65648 2.38647L2.53148 3.48335C1.91273 4.07397 1.8846 5.08647 2.50335 5.70522L2.5596 5.7896C2.64398 5.87397 2.6721 6.0146 2.61585 6.09897C2.5596 6.21147 2.53148 6.29585 2.47523 6.40835C2.41898 6.52085 2.3346 6.5771 2.19398 6.5771H2.1096C1.68773 6.5771 1.29398 6.71772 0.984604 7.0271C0.675229 7.30835 0.506479 7.7021 0.506479 8.12397L0.478354 9.69897C0.450229 10.5708 1.15335 11.274 2.02523 11.3021H2.1096C2.25023 11.3021 2.36273 11.3865 2.39085 11.499C2.4471 11.5833 2.50335 11.6677 2.53148 11.7802C2.5596 11.8927 2.53148 12.0052 2.4471 12.0896L2.39085 12.1458C2.08148 12.4271 1.91273 12.8208 1.91273 13.2427C1.91273 13.6646 2.05335 14.0583 2.36273 14.3677L3.4596 15.4927C4.05023 16.1115 5.06273 16.1396 5.68148 15.5208L5.76585 15.4646C5.85023 15.3802 5.99085 15.3521 6.13148 15.4083C6.24398 15.4646 6.35648 15.5208 6.4971 15.549C6.63773 15.6052 6.7221 15.7177 6.7221 15.8302V15.9427C6.7221 16.8146 7.42523 17.5177 8.2971 17.5177H9.8721C10.744 17.5177 11.4471 16.8146 11.4471 15.9427V15.8302C11.4471 15.7177 11.5315 15.6052 11.644 15.549C11.7002 15.5208 11.7284 15.5208 11.7846 15.4927C11.9252 15.4365 12.0377 15.4646 12.1221 15.549L12.1784 15.6333C12.4596 15.9427 12.8534 16.1115 13.2752 16.1115C13.6971 16.1115 14.0909 15.9708 14.4002 15.6615L15.5252 14.5646C16.144 13.974 16.1721 12.9615 15.5534 12.3427L15.4971 12.2583C15.4127 12.174 15.3846 12.0333 15.4409 11.949C15.4971 11.8365 15.5252 11.7521 15.5815 11.6396C15.6377 11.5271 15.7502 11.4708 15.8627 11.4708H15.9471H15.9752C16.819 11.4708 17.5221 10.7958 17.5502 9.92397L17.5784 8.34897C17.5221 8.01147 17.3534 7.5896 17.0721 7.30835ZM16.2284 9.9521C16.2284 10.1208 16.0877 10.2615 15.919 10.2615H15.8346H15.8065C15.1596 10.2615 14.569 10.6552 14.344 11.2177C14.3159 11.3021 14.2596 11.3865 14.2315 11.4708C13.9784 12.0333 14.0909 12.7365 14.5409 13.1865L14.5971 13.2708C14.7096 13.3833 14.7096 13.5802 14.5971 13.6927L13.4721 14.7896C13.3877 14.874 13.3034 14.874 13.2471 14.874C13.1909 14.874 13.1065 14.874 13.0221 14.7896L12.9659 14.7052C12.5159 14.2271 11.8409 14.0865 11.2221 14.3677L11.1096 14.424C10.4909 14.6771 10.0971 15.2396 10.0971 15.8865V15.999C10.0971 16.1677 9.95648 16.3083 9.78773 16.3083H8.21273C8.04398 16.3083 7.90335 16.1677 7.90335 15.999V15.8865C7.90335 15.2396 7.5096 14.649 6.89085 14.424C6.80648 14.3958 6.69398 14.3396 6.6096 14.3115C6.3846 14.199 6.1596 14.1708 5.9346 14.1708C5.54085 14.1708 5.1471 14.3115 4.83773 14.6208L4.78148 14.649C4.66898 14.7615 4.4721 14.7615 4.3596 14.649L3.26273 13.524C3.17835 13.4396 3.17835 13.3552 3.17835 13.299C3.17835 13.2427 3.17835 13.1583 3.26273 13.074L3.31898 13.0177C3.7971 12.5677 3.93773 11.8646 3.6846 11.3021C3.65648 11.2177 3.62835 11.1333 3.5721 11.049C3.3471 10.4583 2.7846 10.0365 2.13773 10.0365H2.05335C1.8846 10.0365 1.74398 9.89585 1.74398 9.7271L1.7721 8.1521C1.7721 8.0396 1.82835 7.98335 1.85648 7.9271C1.8846 7.89897 1.96898 7.84272 2.08148 7.84272H2.16585C2.81273 7.87085 3.40335 7.4771 3.65648 6.88647C3.6846 6.8021 3.74085 6.71772 3.76898 6.63335C4.0221 6.07085 3.9096 5.36772 3.4596 4.91772L3.40335 4.83335C3.29085 4.72085 3.29085 4.52397 3.40335 4.41147L4.52835 3.3146C4.61273 3.23022 4.6971 3.23022 4.75335 3.23022C4.8096 3.23022 4.89398 3.23022 4.97835 3.3146L5.0346 3.39897C5.4846 3.8771 6.1596 4.01772 6.77835 3.7646L6.89085 3.70835C7.5096 3.45522 7.90335 2.89272 7.90335 2.24585V2.13335C7.90335 2.02085 7.9596 1.9646 7.98773 1.90835C8.01585 1.8521 8.10023 1.82397 8.21273 1.82397H9.78773C9.95648 1.82397 10.0971 1.9646 10.0971 2.13335V2.24585C10.0971 2.89272 10.4909 3.48335 11.1096 3.70835C11.194 3.73647 11.3065 3.79272 11.3909 3.82085C11.9815 4.1021 12.6846 3.9896 13.1627 3.5396L13.2471 3.48335C13.3596 3.37085 13.5565 3.37085 13.669 3.48335L14.7659 4.60835C14.8502 4.69272 14.8502 4.7771 14.8502 4.83335C14.8502 4.8896 14.8221 4.97397 14.7659 5.05835L14.7096 5.1146C14.2034 5.53647 14.0627 6.2396 14.2877 6.8021C14.3159 6.88647 14.344 6.97085 14.4002 7.05522C14.6252 7.64585 15.1877 8.06772 15.8346 8.06772H15.919C16.0315 8.06772 16.0877 8.12397 16.144 8.1521C16.2002 8.18022 16.2284 8.2646 16.2284 8.3771V9.9521Z"
                        fill=""
                      />
                      <path
                        d="M9.00029 5.22705C6.89092 5.22705 5.17529 6.94268 5.17529 9.05205C5.17529 11.1614 6.89092 12.8771 9.00029 12.8771C11.1097 12.8771 12.8253 11.1614 12.8253 9.05205C12.8253 6.94268 11.1097 5.22705 9.00029 5.22705ZM9.00029 11.6114C7.59404 11.6114 6.44092 10.4583 6.44092 9.05205C6.44092 7.6458 7.59404 6.49268 9.00029 6.49268C10.4065 6.49268 11.5597 7.6458 11.5597 9.05205C11.5597 10.4583 10.4065 11.6114 9.00029 11.6114Z"
                        fill=""
                      />
                    </g>
                    <defs>
                      <clipPath id="clip0_130_9763">
                        <rect
                          width="18"
                          height="18"
                          fill="white"
                          transform="translate(0 0.052124)"
                        />
                      </clipPath>
                    </defs>
                  </svg>
                  <span>Setting</span>
                </li>
              </NavLink>
              <li className="logout" onClick={logout}>
                {!loading ? (
                  <>
                    <i className="fa fa-sign-out" aria-hidden="true"></i>
                    Logout
                  </>
                ) : (
                  <span className="loading loading-spinner"></span>
                )}
              </li>
            </ul>
          </div>
        </div>
      </div>
      <div className="main-screen">
        {/* <Router> */}
        <div className="upper-part">{/* <h1>Taskbar</h1> */}</div>
        <div className="lower-part">
          <Routes>
            <Route
              exact
              path="/home"
              element={authUser ? <Home /> : <Navigate to={"/"} />}
            />
            <Route element={<ProtectedRoute allowedRoles={["Admin"]} />}>
              <Route
                exact
                path="/add_dept"
                element={authUser ? <Add_dept /> : <Navigate to={"/"} />}
              />
              <Route
                exact
                path="/add_assetType"
                element={authUser ? <Add_asset_type /> : <Navigate to={"/"} />}
              />
              <Route
                exact
                path="/departments"
                element={authUser ? <Departments /> : <Navigate to={"/"} />}
              /><Route
              exact
              path="/asset_type"
              element={authUser ? <Asset_type /> : <Navigate to={"/"} />}
            />
            </Route>
            <Route
              element={
                <ProtectedRoute allowedRoles={["Admin", "IT-Person", "HR"]} />
              }
            >
              <Route
                exact
                path="/users"
                element={authUser ? <Users /> : <Navigate to={"/"} />}
              />
               <Route
              exact
              path="/users/user_detail"
              element={authUser ? <User_detail /> : <Navigate to={"/"} />}
            />
              <Route
                exact
                path="/total_asset"
                element={authUser ? <Total_asset /> : <Navigate to={"/"} />}
              />
            </Route>
            <Route
              element={
                <ProtectedRoute allowedRoles={["Admin", "IT-Person", "HR"]} />
              }
            >
            <Route
                exact
                path="/total_asset/asset_detail"
                element={authUser ? <Asset_detail /> : <Navigate to={"/"} />}
              />
              </Route>
            <Route
              exact
              path="/profile"
              element={authUser ? <Profile /> : <Navigate to={"/"} />}
            />
            <Route element={<ProtectedRoute allowedRoles={["Employee"]} />}>
              <Route
                exact
                path="/request_asset"
                element={authUser ? <Request_asset /> : <Navigate to={"/"} />}
              />
               <Route
                exact
                path="/request_software"
                element={authUser ? <Request_software /> : <Navigate to={"/"} />}
              />
              <Route
                exact
                path="/request_maintenance"
                element={
                  authUser ? <Request_maintenance /> : <Navigate to={"/"} />
                }
              />
               <Route
                exact
                path="/asset_return"
                element={
                  authUser ? <Asset_return /> : <Navigate to={"/"} />
                }
              />
            </Route>
            <Route element={<ProtectedRoute allowedRoles={["Admin", "HR"]} />}>
              <Route
                exact
                path="/asset_allocation"
                element={
                  authUser ? <Asset_allocation /> : <Navigate to={"/"} />
                }
              />
              <Route
                exact
                path="/add_asset"
                element={authUser ? <Add_asset /> : <Navigate to={"/"} />}
              />
              <Route
                exact
                path="/add_employee"
                element={authUser ? <Add_employee /> : <Navigate to={"/"} />}
              />
            </Route>
            <Route element={<ProtectedRoute allowedRoles={["IT-Person"]} />}>
              <Route
                exact
                path="/add_software"
                element={authUser ? <Add_software /> : <Navigate to={"/"} />}
              />
              <Route
                exact
                path="/temp_allocation"
                element={authUser ? <Temp_allocation /> : <Navigate to={"/"} />}
              />
            </Route>
            <Route
              element={
                <ProtectedRoute
                  allowedRoles={["Employee", "IT-Person", "HR"]}
                />
              }
            >
              <Route
                exact
                path="/feedback"
                element={authUser ? <Feedback /> : <Navigate to={"/"} />}
              />
            </Route>
            <Route
              exact
              path="/notification/*"
              element={authUser ? <Notification /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/notification/hr_notification_detail"
              element={authUser ? <HR_Notification_detail /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/notification/it_notification_detail"
              element={authUser ? <IT_Notification_detail /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/notification/it_issue_detail"
              element={authUser ? <IT_issue_detail /> : <Navigate to={"/"} />}
            />
             <Route
              exact
              path="/notification/it_return_detail"
              element={authUser ? <IT_return_detail /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/notification/emp_notification_detail"
              element={authUser ? <Emp_Notification_detail /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/notification/emp_issue_detail"
              element={authUser ? <Emp_issue_detail /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/notification/emp_return_detail"
              element={authUser ? <Emp_return_detail /> : <Navigate to={"/"} />}
            />
            <Route
              exact
              path="/setting"
              element={authUser ? <Setting /> : <Navigate to={"/"} />}
            />
            {/* <Route exact path="/signup" element={authUser ? <Navigate to="/" /> : <SignUp />}/> */}
          </Routes>
        </div>
        {/* </Router> */}
      </div>
    </div>
  );
};

export default Dash;
